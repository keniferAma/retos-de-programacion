...
def analizador_de_texto_v2(path: str) -> None:
    ...
    for i in range(0, len(split)):
        ...
        longitud_palabras_individuales.append(len(lista_limpia[i]))

        if i >= 1:
            if len(lista_limpia[i]) > len(palabra_mas_larga[0]):
                palabra_mas_larga = [lista_limpia[i]]
    ...
    return {"numero total de palabras": numero_total_de_palabras,
            "numero total de oraciones": numero_de_oraciones,
            "palabra mas larga": ', '.join(palabra_mas_larga),
            "longitud media de las palabras": longitud_media_de_las_palabras}
